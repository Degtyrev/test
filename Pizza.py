...
class Pizza:
    ...
    def __init__(self, name="Маргарита", dough="Традиционное",
                 ...
                 price=300, time_preparing=datetime.timedelta(minutes=30), id=1):
        self.name = name
        self.dough = dough
        self.sauce = sauce
    ...
    def add_filling(self):  # метод добавление нового ингредиента
        ...
        print(f"\nCостав пиццы {self.name}: {self.print_list_composition(self.filling)}")
        ...
        res = input("\nХотите добавить новый ингредиент в пиццу? Цена будет изменена. да/нет").lower().strip()
        while res != "нет":
            if res == "да":
    ...
    def del_filling(self):  # метод удаления нового ингредиента
        ...
        res = input("\nХотите удалить ингредиент из пиццы? Цена будет изменена. да/нет").lower().strip()
        while res != "нет":
            if res == "да":
                print(f"\nCостав пиццы {self.name}: {self.print_list_composition(self.filling)}")
                ...
                if del_item in self.possible_doping.keys():  # если удаляемый элемент в списке дополнительных ингредиентов изменяем цену
                    ...
                    print(f"\nНовый состав пиццы {self.name}: {self.print_list_composition(self.filling)} \n")
                    res = input("\nХотите ещё удалить ингредиент? Цена будет изменена. да/нет").lower().strip()
                    ...
                    res = input("\nХотите удалить ингредиент? Цена будет изменена. да/нет").lower().strip()
            else:
                print("Нет такой опции.")
                res = input("\nХотите удалить ингредиент? Цена будет изменена. да/нет").lower().strip()
...
class PizzaBBQ(Pizza):
    def __init__(self, name="Пицца Барбекю", dough="Традиционное", sauce="Барбекю",
                 ...
                 price=654, time_preparing=datetime.timedelta(minutes=48), id=2):
...
class PizzaPepperoni(Pizza):
    def __init__(self, name="Пицца Пепперони", dough="Тонкое", sauce="Томатный",
                 ...
                 price=520, time_preparing=datetime.timedelta(minutes=30), id =3):
        super().__init__(name, dough, sauce, filling, price, time_preparing, id)
...
class PizzaSeafood(Pizza):
    def __init__(self, name="Пицца с морепродуктами", dough="Традиционное",
                 ...
                 price=983, time_preparing=datetime.timedelta(minutes=36), id=4):
        super().__init__(name, dough, sauce, filling, price, time_preparing, id)
